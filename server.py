...
def lease_cleaner():
    while True:
        now = time.time()
        expired = [c for c, (ip, end) in assigned.items() if now > end]

        for cname in expired:
            ip, _ = assigned.pop(cname)
            ip_pool.append(ip)
            print(f"[LEASE EXPIRED] {cname} → {ip} returned to pool")
...
while True:
    ...
    if data == "DISCOVER":
        ...
        if req_ip not in ip_pool:
            conn.send("NACK".encode())
            print(f"[NACK] {cname} requested unavailable IP {req_ip}")
        else:
    ...
    if assigned:
        for cname, (ip, end) in assigned.items():
            ...
            print(f"{cname:10} → {ip} | {remaining}s left")
    else:
